# PB’s ChatAssistant 1.14.5

1.14.5ではギルド・オフィサーチャンネルの動的な名前を取得し、HUD・通知・ログの表示に共通で使用します。
名前の取得前には `/g1` などのチャンネルコマンドを代わりに表示し、`nil` を表示しません。
1.14.0の操作が実機で動作したとの報告を受け、入力の仕組みは維持しています。
...
1.14.0で切り替えが動作したとの実機報告を受けています。1.14.5ではチャンネル名の取得だけを修正しました。
